 ...
 */
public class DragLayout extends RelativeLayout implements MoveLayout.DeleteMoveLayout {
    ...
    private boolean mIsAddDeleteView = true;
    ...
    private void init(Context c, DragLayout thisp) {
        mContext = c;
    ...
    public void addDragView(View selfView, int left, int top, int right, int bottom, boolean isFixedSize, boolean whitebg, int minwidth, int minheight) {
        ...
        final MoveLayout moveLayout = new MoveLayout(mContext);

        moveLayout.setMoveToDelete(false);
        ...
        moveLayout.setIdentity(((ShapeView)selfView).getProperty().getIdentity());
//        Log.d("cloneid", "cloneid>>movelayout in draglayout: " + ((ShapeView)selfView).getIdentity()+1);
